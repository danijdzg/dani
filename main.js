...
// EN main.js - REEMPLAZA ESTA FUNCIÓN COMPLETA
const handleSaveMovement = async (form, btn) => {
    // ... (la lógica interna de esta función que ya te pasé antes es correcta, pero la incluyo de nuevo para asegurar que tienes la versión final completa y funcional)
    clearAllErrors(form.id);
    ...
    if (isRecurrent) {
        try {
            ...
            // NUEVA LÓGICA: Capturamos los días de la semana
            if (dataToSave.frequency === 'weekly') {
                dataToSave.daysOfWeek = Array.from(selectAll('#recurrent-week-days input:checked')).map(cb => cb.value);
            } else {
                dataToSave.daysOfWeek = null;
            }
            ...
            const activePage = document.querySelector('.view--active');
            if (activePage && (activePage.id === PAGE_IDS.ESTRATEGIA || activePage.id === PAGE_IDS.DIARIO)) {
                renderEstrategiaPlanificacion();
            }
            return true;
        ...
        // ... (la lógica para movimientos normales que te pasé antes es correcta y se mantiene)
        const mode = select('movimiento-mode').value;
